#include "cse7766agg.h"
#include "esphome/core/log.h"

namespace esphome {
namespace cse7766agg {

static const char *TAG = "cse7766agg";

void CSE7766AGGComponent::loop() {
...
float CSE7766AGGComponent::get_setup_priority() const { return setup_priority::DATA; }

bool CSE7766AGGComponent::check_byte_() {
...
void CSE7766AGGComponent::parse_data_() {
  ...
  if ((adj & 0x20) == 0x20 && current_ok && voltage_ok && power != 0.0) {
    ...
    if(this->current_max_sensor_ != nullptr) {
        this->current_max_acc_ = max(current_calib / float(current_cycle), this->current_max_acc_);
    }
    if(this->current_min_sensor_ != nullptr) {
        this->current_min_acc_ = min(current_calib / float(current_cycle), this->current_min_acc_);
    }

  }
}
void CSE7766AGGComponent::update() {
  ...
  float current_max = this->current_counts_ > 0 ? this->current_max_acc_ : 0.0;
  float current_min = this->current_counts_ > 0 ? this->current_min_acc_ : 0.0;
  ...
  if (this->current_max_sensor_ != nullptr)
    this->current_max_sensor_->publish_state(current_max);
  if (this->current_min_sensor_ != nullptr)
    this->current_min_sensor_->publish_state(current_min);
  if (this->power_sensor_ != nullptr)
    this->power_sensor_->publish_state(power);

  this->voltage_acc_ = 0.0f;
  this->current_acc_ = 0.0f;
  this->current_max_acc_ = 0.0f;
  this->current_min_acc_ = 9999.0f;
...
uint32_t CSE7766AGGComponent::get_24_bit_uint_(uint8_t start_index) {
...
void CSE7766AGGComponent::dump_config() {
